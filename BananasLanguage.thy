...
theorem progress: "e \<turnstile> t\<^sub>1 \<rightarrow> t\<^sub>2 \<Longrightarrow> v \<turnstile> t\<^sub>1 \<Longrightarrow> \<exists>v'. v ~ e \<leadsto> v'"
  proof (induction e t\<^sub>1 t\<^sub>2 arbitrary: v rule: typecheck\<^sub>e.induct)
  case tc_id
    have "v ~ \<epsilon> \<leadsto> v" by simp
    thus ?case by fastforce
  next case tc_fst
    then obtain v\<^sub>1 v\<^sub>2 where "v = PairV v\<^sub>1 v\<^sub>2" using canonical_prod by blast
    hence "v ~ \<pi>\<^sub>1 \<leadsto> v\<^sub>1" by simp
    thus ?case by fastforce
  next case tc_snd
    then obtain v\<^sub>1 v\<^sub>2 where "v = PairV v\<^sub>1 v\<^sub>2" using canonical_prod by blast
    hence "v ~ \<pi>\<^sub>2 \<leadsto> v\<^sub>2" by simp
    thus ?case by fastforce
  next case (tc_tup f\<^sub>1 t\<^sub>1 t\<^sub>2 f\<^sub>2 t\<^sub>3)
    then obtain v\<^sub>1 where "v ~ f\<^sub>1 \<leadsto> v\<^sub>1" by blast
    moreover from tc_tup obtain v\<^sub>2 where "v ~ f\<^sub>2 \<leadsto> v\<^sub>2" by blast
    ultimately have "v ~ f\<^sub>1 \<triangle> f\<^sub>2 \<leadsto> PairV v\<^sub>1 v\<^sub>2" by simp
    thus ?case by fastforce
  next case tc_inl
    have "v ~ \<iota>\<^sub>l \<leadsto> InlV v" by simp
    thus ?case by fastforce
  next case tc_inr
    have "v ~ \<iota>\<^sub>r \<leadsto> InrV v" by simp
    thus ?case by fastforce
  next case (tc_case f\<^sub>l t\<^sub>1 t\<^sub>3 f\<^sub>r t\<^sub>2)
    ...
    thus ?case
      ...
      case True
        with tc_case obtain v\<^sub>1 where "v' ~ f\<^sub>l \<leadsto> v\<^sub>1" by blast 
        with True have "v ~ f\<^sub>l \<nabla> f\<^sub>r \<leadsto> v\<^sub>1" by simp
        thus ?thesis by fastforce
      next case False
        from tc_case False V obtain v\<^sub>2 where "v' ~ f\<^sub>r \<leadsto> v\<^sub>2" by blast 
        with False V have "v ~ f\<^sub>l \<nabla> f\<^sub>r \<leadsto> v\<^sub>2" by auto
        thus ?thesis by fastforce
      qed
  next case (tc_outj f)
    then obtain v' where "v = InjV f v'" using canonical_mu by blast
    hence "v ~ Outj f \<leadsto> v'" by simp
    thus ?case by fastforce
  qed
